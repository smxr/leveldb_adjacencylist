...
int main(int argc, char **argv) {
  ...
  options.filter_policy = NewBloomFilterPolicy(10);       //default 10 bits

  options.write_buffer_size = 1024*1024*1024;
  options.block_size = 4 * 1024 * 1024;
  options.max_file_size = 1024* 1024 * 1024;
  options.block_cache = NewLRUCache(1024* 1024 * 1024);
...
#pragma omp parallel for
    for (int i = 0; i < this_s_count; i++) {                          //insert new edges
      ...
      for (int k = 0; k < 2; k++) {
        ...
        string str_key = to_string(pid)+"-"+to_string(target)+"_"+to_string(meetings[i].start)+"~"+to_string(meetings[i].end);
        str_key.resize(40,'x');
        string temp_string = Pack(temp);
        //        assert(sizeof(temp)==48);
        //        assert(temp_string.size()==48);
